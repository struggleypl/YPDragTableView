...
typedef enum{
    MSSnapshotMeetsEdgeTop,
    MSSnapshotMeetsEdgeBottom,
}MSSnapshotMeetsEdge;

@interface MSDragTableView ()
/**对被选中的cell的截图*/
...
/**自动滚动的方向*/
@property (nonatomic, assign) MSSnapshotMeetsEdge autoScrollDirection;

@end


@implementation MSDragTableView
...
- (void)longPressGestureRecognized:(id)sender{
    ...
    switch (longPressState) {
        case UIGestureRecognizerStateBegan:{  //手势开始，对被选中cell截图，隐藏原cell
            ...
            UIButton *btn = (UIButton *)[cell viewWithTag:2018];
            btn.hidden = YES;
            if (_originalIndexPath) {
                [self cellSelectedAtIndexPath:_originalIndexPath];
            }
            if (self.delegate&&[self.delegate respondsToSelector:@selector(cellWillMoveInTableView:)]) {
                [self.delegate cellWillMoveInTableView:self];
            }
            break;
        }
        case UIGestureRecognizerStateChanged:{//点击位置移动，判断手指按住位置是否进入其它indexPath范围，若进入则更新数据源并移动cell
            ...
            if (self.delegate&&[self.delegate respondsToSelector:@selector(cellIsMovingInTableView:)]) {
...
- (void)cellSelectedAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.snapshot.transform = CGAffineTransformMakeScale(1.03, 1.03);
...
- (void)didEndDraging{
    ...
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.snapshot.center = cell.center;
        weakSelf.snapshot.alpha = 0;
        weakSelf.snapshot.transform = CGAffineTransformIdentity;
        cell.alpha = 1;
    } completion:^(BOOL finished) {
        cell.hidden = NO;
        [weakSelf.snapshot removeFromSuperview];
        weakSelf.snapshot = nil;
        weakSelf.originalIndexPath = nil;
        weakSelf.relocatedIndexPath = nil;
    }];
}
...
- (UIView *)customSnapshotFromView:(UIView *)inputView {
    ...
    snapshot.layer.borderColor = [UIColor yellowColor].CGColor;
...
- (BOOL)checkIfSnapshotMeetsEdge{
    ...
    if (minY < self.contentOffset.y) {
        _autoScrollDirection = MSSnapshotMeetsEdgeTop;
        return YES;
    }
    if (maxY > self.bounds.size.height + self.contentOffset.y) {
        _autoScrollDirection = MSSnapshotMeetsEdgeBottom;
        return YES;
    }
    return NO;
}
...
- (void)startAutoScroll{
    ...
    if (_autoScrollDirection == MSSnapshotMeetsEdgeTop) {//向下滚动
